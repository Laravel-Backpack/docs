...
// aditionally you should be able to change Filter attributes by appending `->methods` in your filter definition. Eg:
$this->crud->addFilter($options, $values, $filter_logic)->type('number');
// this would set the filter attribute `type` to be equal to `number`. 
```
